...
object feature {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data  = sc.textFile(path)
      ...
      .map(_.split(',') match { case Array(user, novel, date, count) => ((novel.toInt, getHour(date)), count.toInt)})

    val grp  = data.reduceByKey((x,y) => x+y)
      .sortByKey()
      .map { case ((novel,hour), count) => (novel, (hour, count))}
      .groupByKey
      .mapValues(_.toList)

    val filter = grp.map { case (novel, item) => (novel, flatList(getFeature(item))) }
